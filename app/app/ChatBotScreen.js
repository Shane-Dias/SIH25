...
export default function ChatBotScreen() {
  ...
  const sendMessage = async () => {
    ...
    try {
      ...
      // Simplify the request structure to minimize potential issues
      const response = await axios.post(`${API_BASE_URL}/api/chat-t/`, {
        user_input: input,
        chat_history: [] // Simplified for debugging
      }, {
      ...
      const botText = response.data && response.data.bot_response 
      ...
      setMessages(msgs => [...msgs, { 
        id: msgs.length, 
        text: `Error connecting to chatbot. ${errorMessage}`, 
        sender: 'bot' 
      }]);
    } finally {
      setLoading(false);
